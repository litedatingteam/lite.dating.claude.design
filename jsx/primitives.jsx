...
function CheckBox({ on, green }) {
  return <span className={`ck-box ${green ? 'green' : ''} ${on ? 'on' : ''}`}>{on && <Icon name="checkSm" size={14} style={{ color: 'white' }} />}</span>;
}
function RadioDot({ on }) {
  return <span className={`rd-box ${on ? 'on' : ''}`}><span className="rd-dot" /></span>;
}

Object.assign(window, { Icon, Logo, Photo, Badge, VerifiedBadge, ChannelBadge, Chip, Switch, Field, Avatar, CheckBox, RadioDot });
